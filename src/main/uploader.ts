...
export class MinidumpUploader {
  ...
  /**
   * Reads a minidump from disk for upload to sentry
   * @param minidumpPath The path to the minidump file
   */
  private async _readMinidump(minidumpPath: string): Promise<Buffer> {
    const maybeMinidumpContent = (await readFileAsync(minidumpPath)) as Buffer;
    if (process.platform !== 'linux') {
      return maybeMinidumpContent;
    }

    // We're going to peek the content to see if this is perhaps a multipart/form-data
    // submission.
    if (maybeMinidumpContent.toString('utf8', 0, 2) !== '--') {
      // This is definitely not a multipart/form-data submission, let's assume its a valid minidump
      return maybeMinidumpContent;
    }

    const first256Chars = maybeMinidumpContent.toString('utf8', 0, 256);
    const regexResults = multipartBoundaryRegex.exec(first256Chars);
    if (!regexResults) {
      // This is likely not a multipart/form-data submission generated by breakpad
      return maybeMinidumpContent;
    }

    const boundary = regexResults[1];
    const minidumpContent: Buffer = await new Promise<Buffer>(res => {
      const busboy = new Busboy({ headers: { 'content-type': `multipart/form-data; boundary=${boundary}` } });

      let busboyContent: Buffer | null = null;
      busboy.on('file', (fieldName: string, fileStream: Readable) => {
        if (fieldName !== 'upload_file_minidump') {
          return;
        }

        const chunks: Buffer[] = [];
        fileStream.on('readable', () => {
          for (let chunk: Buffer = fileStream.read(); chunk; chunk = fileStream.read()) {
            chunks.push(chunk);
          }
        });
        ...
        fileStream.on('end', () => {
          if (chunks.length === 0) {
            // this is the best we can do, hope that this is indeed a minidump
            busboyContent = maybeMinidumpContent;
          } else if (chunks.length === 1) {
            busboyContent = chunks[0];
          } else {
            busboyContent = Buffer.concat(chunks);
          }
        });
      });

      busboy.on('finish', () => {
        res(busboyContent || maybeMinidumpContent);
      });

      busboy.write(maybeMinidumpContent);
      busboy.end();
    });

    return minidumpContent;
  }
  ...
  private async _toMinidumpRequest(
    ...
    if (!transport.isRateLimited('attachment')) {
      const minidumpContent = await this._readMinidump(minidumpPath);
